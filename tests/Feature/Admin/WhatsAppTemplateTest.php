...
it('has no create or delete route for whatsapp templates', function () {
    expect(Route::has('admin.whatsapp-template.store'))->toBeFalse();
    expect(Route::has('admin.whatsapp-template.destroy'))->toBeFalse();
});
...
it('uses an admin-edited reminder_sesi_h1 template body for the next reminder sent, not the original seeded text', function () {
    ...
    $siswa = Siswa::factory()->create(['lembaga_id' => $lembaga->id, 'nama_lengkap' => 'Siswa Template Edit']);
