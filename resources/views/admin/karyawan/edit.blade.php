...
<x-app-layout>
    <div class="mx-auto max-w-6xl space-y-6" x-data="{
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <p class="text-sm text-gray-500">
                    ...
                    <span class="mx-1 text-gray-300">&rsaquo;</span> <b class="font-semibold text-gray-900">Detail & Profil Karyawan</b>
                </p>
            </div>
            ...
            <div class="relative flex flex-col gap-6 md:flex-row md:items-center">
                ...
                <div class="flex-1 space-y-2">
                    <div class="flex flex-wrap items-center gap-3">
                        <h1 class="font-display text-2xl font-bold tracking-tight text-gray-900">{{ $karyawan->nama }}</h1>
                        @php
                        ...
                        <span class="rounded-full border px-3 py-0.5 text-xs font-semibold {{ $statusBadge }}">
                            {{ ucwords(str_replace('_', ' ', $karyawan->status_aktif)) }}
                        </span>
                    </div>
                    <div class="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-600">
                        <span class="flex items-center gap-1.5 font-mono">
                            ...
                            NIK: <strong class="text-gray-900">{{ $karyawan->nik ?: '-' }}</strong>
                        </span>
                        <span class="flex items-center gap-1.5">
                            <x-icon name="work" class="h-4 w-4 text-gray-400" />
                            Jabatan: <strong class="text-gray-900">{{ $karyawan->jenisKaryawan->nama ?? '-' }}</strong>
                        </span>
                        @if ($karyawan->user)
                            <span class="flex items-center gap-1.5">
                                <x-icon name="check_circle" class="h-4 w-4 text-brand-600" />
                                <span class="text-xs text-gray-500">Akun Terhubung: {{ $karyawan->user->email ?? $karyawan->user->username }}</span>
                            </span>
                        @endif
                    </div>
                </div>
        ...
        <div class="mt-6">
            @include('admin.karyawan.tabs.profil')
        </div>
    </div>
</x-app-layout>
