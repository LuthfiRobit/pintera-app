<x-app-layout>
    <div class="mx-auto max-w-6xl space-y-4" x-data="jenisKaryawanMasterSPA()">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <p class="text-sm text-gray-500">
                ...
                <span class="mx-1 text-gray-300">&rsaquo;</span> <b class="font-semibold text-gray-700">Jenis Karyawan</b>
            </p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
            ...
            <div class="flex items-center justify-between rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-card transition hover:shadow-elevated">
                <div class="flex items-center gap-3">
                    <span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-indigo-50 text-indigo-600">
                        <x-icon name="verified_user" class="h-5 w-5" />
                    </span>
                    <div>
                        <p class="font-display text-[11px] font-semibold uppercase tracking-wider text-indigo-600">Terpakai</p>
                        <p class="font-display text-lg font-bold text-gray-900 leading-tight" x-text="countTerpakai"></p>
                    </div>
                </div>
                <span class="text-[11px] font-medium text-gray-400">Sedang Digunakan</span>
            </div>
            ...
            <div class="flex items-center justify-between rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-card transition hover:shadow-elevated">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-display text-[11px] font-semibold uppercase tracking-wider text-amber-600">Belum Terpakai</p>
                        <p class="font-display text-lg font-bold text-gray-900 leading-tight" x-text="countKosong"></p>
                    </div>
                </div>
                <span class="text-[11px] font-medium text-gray-400">Siap Dialokasikan</span>
            </div>
        </div>

        {{-- Filter Card (Mobile-first responsive architecture) --}}
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-card">
            <div class="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                @can('jenis-karyawan-master.create')
                <x-tooltip text="Tambah referensi jenis spesialisasi karyawan baru">
                    <button @click="openModal(false)" type="button" class="inline-flex w-full items-center justify-center gap-1.5 rounded-lg bg-brand-500 px-4 py-2 text-xs sm:text-sm font-semibold text-white shadow-sm transition hover:bg-brand-600 active:scale-[0.98] focus:outline-none focus:ring-2 focus:ring-brand-500 focus:ring-offset-2 sm:w-auto">
                        <span class="text-base leading-none">+</span> Tambah Jenis Karyawan
                    </button>
                </x-tooltip>
                @endcan
            </div>
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:items-end">
                ...
                <div class="lg:col-span-5">
                    <label for="search" class="mb-1.5 block text-xs font-semibold text-gray-500">Cari Jenis Karyawan</label>
                    <div class="flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
                        <x-icon name="search" class="h-[13px] w-[13px] shrink-0 text-gray-400" />
                        <input x-model="searchQuery" @input="currentPage = 1" type="text" placeholder="Cari berdasarkan nama jenis karyawan..." class="w-full border-0 bg-transparent p-0 text-xs sm:text-sm text-gray-900 placeholder:text-gray-400 focus:ring-0">
                    </div>
                </div>

                {{-- Filter Tabs (Mobile: flex wrap/scroll, Desktop: 7 cols) --}}
                <div class="lg:col-span-7">
                    <label class="mb-1.5 block text-xs font-semibold text-gray-500">Filter Penggunaan</label>
                    <div class="flex items-center gap-2 overflow-x-auto scrollbar-none pb-1 sm:pb-0">
                        ...
                        <button @click="activeFilter = 'terpakai'; currentPage = 1" type="button" :class="activeFilter === 'terpakai' ? 'bg-indigo-50 font-semibold text-indigo-700 border-indigo-200 shadow-2xs' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 hover:text-gray-900 border-gray-200'" class="flex-1 sm:flex-none justify-center px-4 py-2 rounded-lg text-xs font-semibold border transition-all whitespace-nowrap flex items-center gap-2">
                            <span>Terpakai</span>
                            <span :class="activeFilter === 'terpakai' ? 'bg-indigo-100/80 text-indigo-700' : 'bg-gray-200 text-gray-700'" class="px-2 py-0.5 text-[10px] rounded-full font-bold" x-text="countTerpakai"></span>
                        </button>
                        <button @click="activeFilter = 'kosong'; currentPage = 1" type="button" :class="activeFilter === 'kosong' ? 'bg-amber-50 font-semibold text-amber-700 border-amber-200 shadow-2xs' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 hover:text-gray-900 border-gray-200'" class="flex-1 sm:flex-none justify-center px-4 py-2 rounded-lg text-xs font-semibold border transition-all whitespace-nowrap flex items-center gap-2">
                            <span>Belum Terpakai</span>
                            <span :class="activeFilter === 'kosong' ? 'bg-amber-100/80 text-amber-700' : 'bg-gray-200 text-gray-700'" class="px-2 py-0.5 text-[10px] rounded-full font-bold" x-text="countKosong"></span>
                        </button>
                    </div>
                </div>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="text-left text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                            ...
                            <th class="px-5 py-3">Nama Jenis Karyawan</th>
                            <th class="px-5 py-3">Status Penggunaan</th>
                            <th class="px-5 py-3">Jumlah Karyawan</th>
                        </tr>
                    </thead>
                        ...
                        <template x-for="(item, index) in paginatedItems" :key="item.id">
                            <tr class="transition hover:bg-gray-50">
                                <td class="sticky left-0 z-10 bg-white px-5 py-3">
                                    <x-table-actions>
                                        @can('jenis-karyawan-master.edit')
                                        ...
                                        @endcan
                                        @can('jenis-karyawan-master.delete')
                                ...
                                <td class="px-5 py-3.5 text-gray-600">
                                    <span :class="item.karyawan_count > 0 ? 'bg-indigo-50 text-indigo-700' : 'bg-amber-50 text-amber-700'" class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold" x-text="item.karyawan_count > 0 ? 'Terpakai' : 'Belum Terpakai'"></span>
                                </td>
                                <td class="px-5 py-3.5 text-gray-600">
                                    <span :class="item.karyawan_count > 0 ? 'bg-blue-100 text-blue-700 font-bold' : 'bg-gray-100 text-gray-600'" class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold" x-text="item.karyawan_count + ' Karyawan'"></span>
                                </td>
                            </tr>
                        </template>
                    </tbody>
        ...
        <div x-show="showModal" class="fixed inset-0 z-50 overflow-y-auto px-4 py-6 sm:px-0 flex items-center justify-center" style="display: none;">
            ...
            <div x-show="showModal" class="bg-white rounded-2xl overflow-hidden shadow-elevated transform transition-all sm:max-w-md sm:w-full z-10 p-6 relative text-left"
                ...
                <div class="flex items-center justify-between pb-3.5 border-b border-gray-200">
                    <h3 class="font-display text-base font-bold text-gray-900" x-text="isEditing ? 'Edit Jenis Karyawan' : 'Tambah Jenis Karyawan'"></h3>
                ...
                <form @submit.prevent="submitForm()" class="mt-4 space-y-4">
                    <div>
                        <label class="mb-1.5 block text-xs font-semibold text-gray-700">Nama Jenis Karyawan <span class="text-error-500">*</span></label>
                        <input x-model="form.nama" type="text" required placeholder="Contoh: Konselor BK / Psikolog" class="w-full rounded-lg border border-gray-200 bg-gray-50 px-3.5 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus:border-brand-500 focus:ring-brand-500">
                    ...
                    <div class="flex items-center justify-end gap-2 pt-4 mt-6 border-t border-gray-100">
                        ...
                        <x-primary-button x-bind:disabled="loading" type="submit">
                            <span x-text="loading ? 'Menyimpan...' : (isEditing ? 'Simpan Perubahan' : 'Rilis Jenis Karyawan')"></span>
                        </x-primary-button>
                    </div>
                </form>
            </div>
    ...
    <script>
    function jenisKaryawanMasterSPA() {
        return {
            items: @json($jenisList ?? []),
            activeFilter: 'semua',
            searchQuery: '',
            perPage: 10,
            currentPage: 1,
            ...
            get countTerpakai() {
                return this.items.filter(i => i.karyawan_count > 0).length;
            },
            
            get countKosong() {
                return this.items.filter(i => i.karyawan_count === 0 || !i.karyawan_count).length;
            },
            
            get filteredItems() {
                let res = this.items;
                if (this.activeFilter === 'terpakai') {
                    res = res.filter(i => i.karyawan_count > 0);
                } else if (this.activeFilter === 'kosong') {
                    res = res.filter(i => i.karyawan_count === 0 || !i.karyawan_count);
                }
            ...
            async submitForm() {
                this.loading = true;
                this.errors = {};
                const url = this.isEditing ? `/admin/jenis-karyawan-master/${this.form.id}` : '/admin/jenis-karyawan-master';
            ...
            async deleteItem(item) {
                if (item.karyawan_count > 0) {
                    this.showToast(`Jenis karyawan ini sedang Dipakai oleh ${item.karyawan_count} Karyawan dan tidak boleh dihapus.`, 'error');
                    return;
                }
                if (!confirm(`Yakin ingin menghapus referensi jenis karyawan "${item.nama}" secara permanen?`)) return;

                try {
                    const response = await fetch(`/admin/jenis-karyawan-master/${item.id}`, {
                        method: 'DELETE',
                        headers: {
                            'Accept': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
                    ...
                    if (response.ok) {
                        ...
                        this.showToast(data.message || 'Jenis karyawan ini masih dipakai oleh karyawan.', 'error');
                    } else {
