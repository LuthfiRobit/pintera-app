<x-app-layout>
    <div class="mx-auto max-w-5xl space-y-5 pt-2">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
                <div class="flex flex-wrap items-center gap-2.5">
                    <h1 class="font-display text-xl font-bold text-gray-900">Nilai &amp; Rapor Anak</h1>
                    @if ($anak?->kelas)
                        <span class="inline-flex items-center gap-1.5 rounded-full border border-brand-200 bg-brand-50 px-2.5 py-0.5 text-xs font-semibold text-brand-700">
                            ...
                            <span>{{ $anak->kelas->nama }}</span>
                        </span>
                    @endif
                </div>
                <p class="mt-1 text-xs text-gray-500">
                    Pantau pencapaian hasil belajar mata pelajaran dan unduh rapor resmi anak Anda.
                </p>
            </div>
            <p class="text-xs text-gray-400 shrink-0">
                Portal Orang Tua <span class="mx-1 text-gray-300">&rsaquo;</span> <b class="font-semibold text-gray-600">Nilai &amp; Rapor</b>
            </p>
        </div>
        ...
        @if ($anakList->isEmpty())
            <div class="rounded-2xl border border-amber-200 bg-amber-50/60 p-6 text-center shadow-xs">
                ...
                <h4 class="mt-3 font-display text-base font-bold text-amber-900">Belum Ada Data Siswa</h4>
                <p class="mt-1 text-xs text-amber-700">Belum ada data siswa yang terhubung dengan akun Anda. Hubungi admin sekolah untuk menghubungkan akun Anda ke data anak.</p>
            </div>
        @else
            {{-- ─── KPI Summary Cards ───────────────────────────────── --}}
            @php
                ...
                <div class="rounded-2xl border border-gray-200 bg-white p-4 shadow-card transition hover:shadow-md">
                    ...
                    <p class="mt-1.5 text-[11px] text-gray-500">
                        @if ($nilaiList->isNotEmpty())
                            Tertinggi {{ $nilaiTertinggi }} &middot; Terendah {{ $nilaiTerendah }}
                        @else
            ...
            {{-- ─── Filter Anak & Semester (TomSelect, No Reload) ──── --}}
            <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-card">
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <x-select
                            id="anak_id_select"
                            name="siswa_id"
                            onchange="window.location.href = '{{ route('admin.nilai-anak.index') }}?siswa_id=' + this.value"
                        >
                            @foreach ($anakList as $anakOpsi)
                                <option value="{{ $anakOpsi->id }}" @selected($anak?->id === $anakOpsi->id)>
                                    {{ $anakOpsi->nama_lengkap }} &middot; {{ $anakOpsi->kelas?->nama ?? 'Belum Ditentukan' }}
                                </option>
                            @endforeach
                        </x-select>
                    </div>
                    <div>
                        ...
                        <x-select
                            ...
                            name="semester_id"
                            onchange="window.location.href = '{{ route('admin.nilai-anak.index') }}?siswa_id={{ $anak?->id }}&semester_id=' + this.value"
                        >
                ...
                @if ($anak?->kelas)
                    <div class="mt-3 flex items-center gap-2 text-xs text-gray-400">
                        ...
                        <span class="font-semibold text-gray-600">{{ $anak->kelas->nama }}</span>
                        @if ($anak->nis)
                            <span class="text-gray-300">&middot;</span>
                            <span>NIS: <b class="font-medium text-gray-700">{{ $anak->nis }}</b></span>
                        @endif
                    </div>
                @endif
            </div>
                ...
                <div class="rounded-2xl border border-emerald-200 bg-gradient-to-r from-emerald-50/60 via-white to-emerald-50/30 p-5 shadow-card">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <button
                            type="button"
                            x-data
                            @click="$store.imagePreview.buka('{{ route('admin.nilai-anak.unduh-rapor', ['siswa' => $anak->id, 'semester_id' => $semesterId]) }}?inline=1', 'Rapor - {{ $anak->nama_lengkap }} ({{ $currentSemester?->nama }})', true)"
            ...
            <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-card">
                <div class="flex items-center justify-between border-b border-gray-200 px-5 py-4">
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-0.5">{{ $anak?->nama_lengkap }} &middot; {{ $currentSemester?->nama ?? 'Semester terpilih' }}</p>
                    </div>
    ...
    {{-- Initialize TomSelect for Anak & Semester Select --}}
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const anakEl = document.getElementById('anak_id_select');
            if (anakEl && window.TomSelect) {
                new window.TomSelect(anakEl, {
                    maxItems: 1,
                    create: false,
            ...
            const semesterEl = document.getElementById('semester_id_select');
            if (semesterEl && window.TomSelect) {
                new window.TomSelect(semesterEl, {
                    maxItems: 1,
                    create: false,
