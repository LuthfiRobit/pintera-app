    ...
    <div class="mx-auto max-w-5xl space-y-6 pt-2">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-sm text-slate">Pantau pencapaian hasil belajar mata pelajaran dan unduh rapor resmi anak Anda.</p>
            </div>
            @if ($anak)
            ...
            <x-panel class="p-6">
                <form method="GET" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    ...
                    <div>
                        ...
                        <select name="semester_id" onchange="this.form.submit()" class="mt-1.5 block w-full rounded-xl border-ink/15 text-sm text-ink shadow-sm transition focus:border-brand-500 focus:ring-brand-500">
            ...
            @if ($pengajuanRapor)
                <x-panel class="border border-brand-200 bg-gradient-to-r from-brand-50/70 via-white to-brand-50/30 p-6">
                    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <a href="{{ route('admin.nilai-anak.unduh-rapor', ['siswa' => $anak->id, 'semester_id' => $semesterId]) }}" target="_blank" class="inline-flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-xs font-semibold text-white shadow-sm hover:bg-brand-700 transition active:scale-95">
            ...
            <x-panel class="p-6">
                <div class="flex items-center justify-between border-b border-ink/10 pb-4">
                    <div>
                        ...
                        <p class="text-xs text-slate">{{ $anak?->nama_lengkap }} &middot; {{ $semesterList->firstWhere('id', $semesterId)?->nama ?? 'Semester Terpilih' }}</p>
                    </div>
