<x-app-layout>
    <div class="mx-auto max-w-6xl space-y-4" x-data="kasusIndexSPA()">
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-0.5">Daftar kasus bimbingan konseling dan psikologi yang relevan untuk Anda.</p>
            </div>
            @can('kasus.ajukan')
                <x-link-button href="{{ route('kasus.create') }}" class="inline-flex items-center justify-center gap-1.5 shadow-sm">
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div class="flex items-center justify-between rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-card transition hover:shadow-elevated">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="font-display text-lg font-bold text-gray-900 leading-tight" x-text="countBerjalan"></p>
                    </div>
                </div>
                <span class="text-[11px] font-medium text-gray-400">Proses Aktif</span>
            </div>
            ...
            <div class="flex items-center justify-between rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-card transition hover:shadow-elevated">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="font-display text-lg font-bold text-gray-900 leading-tight" x-text="countButuhTindakan"></p>
                    </div>
                </div>
                <span class="text-[11px] font-medium text-amber-600 font-semibold">Triase / Consent</span>
            </div>
            ...
            <div class="flex items-center justify-between rounded-xl border border-gray-200 bg-white px-4 py-3 shadow-card transition hover:shadow-elevated">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <p class="font-display text-[11px] font-semibold uppercase tracking-wider text-red-600">Eskalasi Kritis</p>
                        <p class="font-display text-lg font-bold text-gray-900 leading-tight" x-text="countEskalasi"></p>
                    </div>
                </div>
                <span class="text-[11px] font-bold text-red-600">Prioritas Atas</span>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-card">
            ...
            <div class="grid grid-cols-1 gap-4 lg:grid-cols-12 lg:items-end">
                ...
                <div class="lg:col-span-4">
                    <label for="search" class="mb-1.5 block text-xs font-semibold text-gray-500">Cari Kasus</label>
                ...
                <div class="lg:col-span-8">
                    <label class="mb-1.5 block text-xs font-semibold text-gray-500">Filter Status Pendampingan</label>
                    <div class="flex items-center gap-2 overflow-x-auto scrollbar-none pb-1 sm:pb-0">
                        ...
                        <button @click="activeFilter = 'diajukan'; currentPage = 1" type="button" :class="activeFilter === 'diajukan' ? 'bg-amber-50 font-semibold text-amber-700 border-amber-200 shadow-2xs' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 hover:text-gray-900 border-gray-200'" class="flex-1 sm:flex-none justify-center px-3.5 py-2 rounded-lg text-xs font-semibold border transition-all whitespace-nowrap flex items-center gap-1.5">
                            <span>Diajukan & Proses</span>
                            <span :class="activeFilter === 'diajukan' ? 'bg-amber-100/80 text-amber-700' : 'bg-gray-200 text-gray-700'" class="px-2 py-0.5 text-[10px] rounded-full font-bold" x-text="countDiajukan"></span>
                        </button>
                        <button @click="activeFilter = 'berjalan'; currentPage = 1" type="button" :class="activeFilter === 'berjalan' ? 'bg-green-50 font-semibold text-green-700 border-green-200 shadow-2xs' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 hover:text-gray-900 border-gray-200'" class="flex-1 sm:flex-none justify-center px-3.5 py-2 rounded-lg text-xs font-semibold border transition-all whitespace-nowrap flex items-center gap-1.5">
                            <span>Berjalan</span>
                            <span :class="activeFilter === 'berjalan' ? 'bg-green-100/80 text-green-700' : 'bg-gray-200 text-gray-700'" class="px-2 py-0.5 text-[10px] rounded-full font-bold" x-text="countBerjalan"></span>
                        </button>
                        <button @click="activeFilter = 'eskalasi'; currentPage = 1" type="button" :class="activeFilter === 'eskalasi' ? 'bg-red-50 font-semibold text-red-700 border-red-200 shadow-2xs' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 hover:text-gray-900 border-gray-200'" class="flex-1 sm:flex-none justify-center px-3.5 py-2 rounded-lg text-xs font-semibold border transition-all whitespace-nowrap flex items-center gap-1.5">
                            <span>Eskalasi</span>
                            <span :class="activeFilter === 'eskalasi' ? 'bg-red-100/80 text-red-700 font-extrabold' : 'bg-gray-200 text-gray-700'" class="px-2 py-0.5 text-[10px] rounded-full font-bold" x-text="countEskalasi"></span>
                        </button>
                        <button @click="activeFilter = 'selesai'; currentPage = 1" type="button" :class="activeFilter === 'selesai' ? 'bg-slate-100 font-semibold text-slate-700 border-slate-300 shadow-2xs' : 'bg-gray-50 text-gray-600 hover:bg-gray-100 hover:text-gray-900 border-gray-200'" class="flex-1 sm:flex-none justify-center px-3.5 py-2 rounded-lg text-xs font-semibold border transition-all whitespace-nowrap flex items-center gap-1.5">
                            <span>Selesai</span>
                            <span :class="activeFilter === 'selesai' ? 'bg-slate-200 text-slate-700' : 'bg-gray-200 text-gray-700'" class="px-2 py-0.5 text-[10px] rounded-full font-bold" x-text="countSelesai"></span>
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-card">
            ...
            <div class="flex flex-col gap-2.5 border-b border-gray-200 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-center gap-2.5">
                    <p class="font-display text-sm font-bold text-gray-900">Daftar Kasus Pendampingan</p>
                        ...
                        <template x-if="paginatedItems.length === 0">
                            <tr>
                                <td colspan="5" class="px-5 py-12 text-center text-gray-400">
                                    ...
                                    <p class="mt-3 text-sm font-semibold text-gray-700" x-text="searchQuery || activeFilter !== 'semua' ? 'Tidak ada data kasus yang cocok dengan filter atau pencarian Anda.' : 'Belum Ada Kasus'"></p>
                                    <p class="mx-auto mt-0.5 max-w-sm text-xs text-gray-400" x-text="searchQuery || activeFilter !== 'semua' ? 'Coba ubah kata kunci atau ganti filter status.' : 'Belum ada catatan atau pengajuan kasus pendampingan terdaftar saat ini.'"></p>
                                </td>
                            </tr>
                        </template>
                        <template x-for="(item, index) in paginatedItems" :key="item.id">
                            <tr :class="item.is_eskalasi ? 'bg-red-50/70 border-l-4 border-red-500 hover:bg-red-100/70 transition' : 'transition hover:bg-gray-50'">
                                ...
                                <td class="px-5 py-3.5">
                                    <p class="font-bold text-gray-900" x-text="item.siswa_nama"></p>
                                    <span x-show="item.is_eskalasi" style="display: none;" class="inline-block mt-0.5 rounded bg-red-200 px-1.5 py-0.5 text-[10px] font-extrabold text-red-900 tracking-wide">PRIORITAS ESKALASI</span>
                                </td>
                                <td class="px-5 py-3.5">
    ...
    @php
        $spaItems = $kasusList->map(function ($k) {
            $val = $k->status->value ?? (string) $k->status;
            $filterGroup = 'lain';
            if (in_array($val, ['diajukan', 'menunggu_consent', 'ditugaskan'], true)) {
                $filterGroup = 'diajukan';
            } elseif ($val === 'berjalan') {
                $filterGroup = 'berjalan';
            } elseif ($val === 'eskalasi') {
                $filterGroup = 'eskalasi';
            } elseif ($val === 'selesai') {
                $filterGroup = 'selesai';
            }
            ...
            return [
                'id' => $k->id,
                'siswa_nama' => $k->siswa->nama_lengkap ?? '-',
                'kategori' => $k->kategori_masalah ?? '-',
                'status_val' => $val,
                'filter_group' => $filterGroup,
                'status_label' => $k->status->label(),
                'badge_tone' => $k->status->badgeTone(),
                'is_eskalasi' => ($val === 'eskalasi'),
                'tanggal' => $k->created_at->format('d M Y'),
                'show_url' => route('kasus.show', $k),
            ];
        })->values();
    @endphp

    <script>
    function kasusIndexSPA() {
        return {
            items: @json($spaItems),
            activeFilter: 'semua',
            searchQuery: '',
            ...
            get countDiajukan() {
                return this.items.filter(i => i.filter_group === 'diajukan').length;
            },
            
            get countBerjalan() {
                return this.items.filter(i => i.filter_group === 'berjalan').length;
            },

            get countEskalasi() {
                return this.items.filter(i => i.filter_group === 'eskalasi').length;
            },

            get countSelesai() {
                return this.items.filter(i => i.filter_group === 'selesai').length;
            },

            get countButuhTindakan() {
                return this.items.filter(i => ['diajukan', 'menunggu_consent', 'eskalasi'].includes(i.status_val)).length;
            },

            get filteredItems() {
                let res = this.items;
                if (this.activeFilter !== 'semua') {
                    res = res.filter(i => i.filter_group === this.activeFilter);
                }
                if (this.searchQuery.trim() !== '') {
                    const q = this.searchQuery.toLowerCase();
                    res = res.filter(i => i.siswa_nama.toLowerCase().includes(q) || i.kategori.toLowerCase().includes(q) || i.status_label.toLowerCase().includes(q));
                }
                return res;
            },
