<x-app-layout>
    <div class="mx-auto max-w-5xl space-y-4">
        ...
        {{-- Information Banner --}}
        <div class="rounded-2xl border border-indigo-200 bg-indigo-50/50 p-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p class="text-base font-extrabold text-indigo-950">Rp {{ number_format($proposal->nominal_pencairan, 0, ',', '.') }}</p>
                </div>
            </div>
            <p class="text-xs text-indigo-700 max-w-xs text-right">
                Input nominal nota riil per barang dan lampirkan scan struk/faktur serta foto fisik barang saat tiba di sekolah.
            </p>
        </div>
        ...
        >
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-6 shadow-card space-y-4">
                <h2 class="font-display text-sm font-bold text-gray-900 border-b border-gray-100 pb-3">Realisasi Harga & Bukti Nota Belanja</h2>
                        ...
                        <div class="rounded-xl border border-gray-200 bg-gray-50/50 p-4 space-y-3">
                            ...
                            <div class="flex items-center justify-between border-b border-gray-200/75 pb-2">
                                <div>
                                    ...
                                    <span class="text-xs text-gray-500 font-medium ml-2" x-text="`(${item.qty} ${item.satuan})`"></span>
                                </div>
                            ...
                            <div class="grid grid-cols-1 gap-3 sm:grid-cols-4">
                                <div>
                                    <label class="block text-[11px] font-semibold text-gray-600 mb-1">Harga Satuan Riil Nota (Rp) <span class="text-error-600">*</span></label>
                                    <input type="number" min="0" :name="`items[${index}][harga_satuan_riil]`" x-model.number="item.harga_satuan_riil" required class="w-full rounded-lg border-gray-200 bg-white text-xs focus:border-brand-500 focus:ring-brand-500">
                                ...
                                <div>
                                    ...
                                    <div class="rounded-lg bg-gray-100 px-3 py-2 text-xs font-bold text-gray-900" x-text="formatRupiah(item.total_riil)"></div>
                                </div>

                                <div>
                                    <label class="block text-[11px] font-semibold text-gray-600 mb-1">Scan Nota / Faktur</label>
                                    <input type="file" :name="`items[${index}][foto_nota]`" accept="image/*,application/pdf" class="w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs file:bg-gray-200">
                                </div>

                                <div>
                                    <label class="block text-[11px] font-semibold text-gray-600 mb-1">Foto Fisik Barang Tiba</label>
                                    <input type="file" :name="`items[${index}][foto_fisik]`" accept="image/*" class="w-full text-xs text-gray-500 file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs file:bg-gray-200">
                                </div>
                            </div>
                        </div>
                    </template>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 pt-2">
                    <div class="rounded-xl border border-gray-200 bg-gray-50 p-4 space-y-1">
                        ...
                        <p class="text-lg font-bold text-gray-900" x-text="formatRupiah(hitungTotalBelanja())"></p>
                    </div>

                    <div class="rounded-xl border p-4 space-y-1" :class="hitungSelisih() >= 0 ? 'border-emerald-200 bg-emerald-50/50' : 'border-rose-200 bg-rose-50/50'">
                        <span class="text-xs font-semibold uppercase tracking-wider" :class="hitungSelisih() >= 0 ? 'text-emerald-700' : 'text-rose-700'" x-text="hitungSelisih() >= 0 ? 'Sisa Kas (Wajib Dikembalikan):' : 'Kekurangan Dana (Defisit):'"></span>
                        <p class="text-lg font-bold" :class="hitungSelisih() >= 0 ? 'text-emerald-900' : 'text-rose-900'" x-text="formatRupiah(Math.abs(hitungSelisih()))"></p>
                    </div>
                </div>

                {{-- Lampiran Bukti Pengembalian Sisa Dana --}}
                <div class="rounded-xl border border-gray-200 p-4 space-y-2" x-show="hitungSelisih() > 0">
