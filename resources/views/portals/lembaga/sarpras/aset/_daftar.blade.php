<div class="rounded-2xl border border-gray-200 bg-white shadow-card">
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-5 py-4">
        <p class="font-display text-sm font-bold text-gray-900">Daftar Aset & Inventaris</p>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="text-left text-[11px] font-semibold uppercase tracking-wide text-gray-500">
                    ...
                    <th class="px-5 py-3">Kode Inventaris</th>
                    <th class="px-5 py-3">Nama Barang & Merk</th>
                    <th class="px-5 py-3">Kategori</th>
                    <th class="px-5 py-3">Lokasi Ruangan</th>
                ...
                @foreach ($asetList as $aset)
                    <tr class="transition hover:bg-gray-50">
                        <td class="sticky left-0 z-10 bg-white px-5 py-3">
                            <x-table-actions>
                                <x-dropdown-link :href="route('admin.sarpras.aset.show', $aset)">
                                    <span class="inline-flex items-center gap-2.5">
                                        <x-icon name="visibility" class="h-4 w-4 text-gray-500" />
                                        Detail & Mutasi
                                    </span>
                                </x-dropdown-link>
                                <x-dropdown-link :href="route('admin.sarpras.aset.edit', $aset)">
                                    <span class="inline-flex items-center gap-2.5">
                        ...
                        <td class="px-5 py-3.5 font-mono font-bold text-brand-600">
                            <a href="{{ route('admin.sarpras.aset.show', $aset) }}" class="hover:underline">
                                {{ $aset->kode_inventaris }}
                            </a>
                        </td>
                        <td class="px-5 py-3.5">
                            <span class="font-semibold text-gray-900">{{ $aset->nama_barang }}</span>
                            @if ($aset->merk)
                                <p class="text-xs text-gray-400 mt-0.5">Merk: {{ $aset->merk }}</p>
                            @endif
                        </td>
                        <td class="px-5 py-3.5 text-gray-600">
                            <x-badge tone="blue">{{ $aset->kategori->nama_kategori ?? '-' }}</x-badge>
                        </td>
                        <td class="px-5 py-3.5 text-gray-600">
                            {{ $aset->ruangan->nama_ruangan ?? '-' }}
                            <p class="text-xs text-gray-400 mt-0.5">{{ $aset->ruangan->gedung->nama_gedung ?? '' }}</p>
                        </td>
                        <td class="px-5 py-3.5 text-center font-semibold text-gray-900">
                            {{ $aset->qty }} {{ $aset->satuan }}
                        </td>
                ...
                @if ($asetList->isEmpty())
                    <tr>
                        <td colspan="7" class="px-5 py-10 text-center text-gray-500">
                            @if (request()->anyFilled(['search', 'kategori_id', 'ruangan_id', 'kondisi']))
    ...
    <div class="border-t border-gray-200 px-5 py-4">
        {{ $asetList->links('pagination.tailadmin') }}
    </div>
</div>
