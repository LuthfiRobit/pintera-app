            ...
            <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                @foreach($weightSummaries as $group)
                    ...
                    <div class="rounded-xl border {{ $isComplete ? 'border-emerald-200 bg-emerald-50/40' : 'border-amber-200 bg-amber-50/40' }} p-3.5 transition">
                        ...
                        <div class="w-full bg-gray-200 rounded-full h-2 mb-1.5 overflow-hidden">
                            <div class="h-2 rounded-full transition-all duration-500 {{ $isComplete ? 'bg-emerald-500' : ($totalBobot > 100 ? 'bg-error-600' : 'bg-amber-500') }}" style="width: {{ min($totalBobot, 100) }}%"></div>
                        </div>
                        <p class="text-[11px] font-semibold {{ $isComplete ? 'text-emerald-700' : 'text-amber-700' }} flex items-center gap-1">
                            @if($isComplete)
                                <span>&checkmark; Genap 100% - Siap Untuk Nilai &amp; Rapor</span>
                            @else
                                <span>&bull; Belum Genap 100% (Kurang {{ max(0, 100 - $totalBobot) }}%)</span>
                            @endif
                        </p>
                    </div>
                @endforeach
