...
**Temuan tambahan (ditemukan saat brainstorming 04d, diperbaiki sekaligus di sub-task ini):** field `kktp_minimal` (ditambahkan ke backend di 04b — migrasi, DTO, Action, FormRequest, seeder) **TIDAK PERNAH ditambahkan ke UI mana pun** — diverifikasi lewat `grep -rn "kktp_minimal" resources/views/` yang hasilnya kosong total. Sejak 04b, field itu hanya bisa diisi lewat seeder/query langsung, tidak ada cara mengisinya dari form. 04d memperbaiki ini SEKALIGUS dengan menambah `elemen_cp`, karena keduanya berakhir di lokasi form yang sama persis:

- `Admin\KomponenPenilaianController::create()`/`edit()` += variabel `$bentukPendidikan` (dari `$request->user()->lembaga->bentuk_pendidikan` — SATU nilai konstan untuk seluruh form, karena `MataPelajaran` yang jadi pilihan sudah otomatis tenant-scoped ke SATU lembaga aktor, tidak perlu logic per-opsi/reaktif JS) diteruskan ke view.
- `Guru\KomponenPenilaianController::create()`/`edit()` += variabel `$bentukPendidikan` yang sama (dari `$request->user()->lembaga->bentuk_pendidikan`).
- Ke-4 view (`admin/komponen-penilaian/{create,edit}.blade.php`, `guru/komponen-penilaian/{create,edit}.blade.php`) += 2 field baru ditambahkan BERSAMA dalam satu blok setelah field "KKTP / Kriteria Ketercapaian" yang sudah ada:
  - `<input type="number" name="kktp_minimal">` (0-100, label "Ambang KKTP Minimal (Opsional)") — SELALU ditampilkan di semua jenjang (bukan kondisional — field ini berlaku universal, beda dari `elemen_cp`).
  - `<select name="elemen_cp">` 3 opsi — ditampilkan HANYA lewat `@if(in_array($bentukPendidikan, ['KB', 'TPA', 'SPS', 'TK']))` (whitelist sama seperti field kondisional 04c; duplikasi literal `in_array`, TIDAK perlu enum/abstraksi baru, per keputusan YAGNI 04c).
  - Value lama diisi via `old('kktp_minimal', $komponenPenilaian->kktp_minimal ?? null)`/`old('elemen_cp', $komponenPenilaian->elemen_cp?->value ?? null)` di `edit.blade.php` (pola sama seperti field `kktp` yang sudah ada di file yang sama).
...
- **Perluasan `elemen_cp` + perbaikan UI `kktp_minimal`** (extend `tests/Feature/Admin/KomponenPenilaianCrudTest.php` dan `tests/Feature/Guru/KomponenPenilaianControllerTest.php`): field `elemen_cp`/`kktp_minimal` tersimpan saat dikirim lewat form, `null` saat tidak dikirim, TIDAK ada perubahan assertion pada test yang sudah ada (backward-compatible, sama seperti pembuktian `kktp_minimal` di 04b). TAMBAHAN baru: assert `create.blade.php`/`edit.blade.php` (Admin & Guru) benar-benar merender `<select name="elemen_cp">` untuk lembaga PAUD dan TIDAK merender untuk lembaga SD/SMP/SMA/SMK (`assertSee`/`assertDontSee` pada `name="elemen_cp"`), dan `<input name="kktp_minimal">` SELALU muncul di semua jenjang.
