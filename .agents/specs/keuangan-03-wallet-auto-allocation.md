...
2. Loop setiap tagihan dalam transaction:
   ...
   - **Jika `$wallet->balance < $sisa` (namun saldo > 0)**:
     - Panggil `$wallet->debit($wallet->balance, $pembayaran, 'Auto-debit parsial tagihan ' . $tagihan->jenisTagihan->nama)`.
     - Buat record `pembayaran` (`metode='wallet_auto'`, `is_auto_allocation=true`, `status='lunas'`).
     - Buat record `pembayaran_tagihan` (`amount_allocated=$wallet->balance`).
     - Update `$tagihan->paid_amount += $wallet->balance` dan set `$tagihan->status = 'sebagian'`.
     - Masukkan sisa tagihan ke daftar `$skipped` sebagai referensi alert, lalu hentikan loop (saldo habis).
...
- [x] Urutan prioritas alokasi ? `priority_score ASC` lalu `jatuh_tempo ASC`.
- [x] Kebijakan saldo kurang ? Partial Top-Down (lunasi sebagian dari saldo yang ada, sisa tagihan menjadi tunggakan). **Catatan (2026-08-11, dikonfirmasi ulang oleh user):** spec awal di `docs/superpowers/specs/2026-08-10-keuangan-03-wallet-auto-allocation-design.md` sempat menyebut kebijakan "skip penuh" (lewati tagihan yang tak bisa dilunasi penuh, coba tagihan berikutnya) sebagai keputusan ADR awal — versi refresh ini MENGGANTI keputusan tsb tanpa penanda eksplisit. Ditemukan saat audit lintas sub-project 1-4, dikonfirmasi ulang ke user: **Partial Top-Down (kode `AutoAllocationEngine.php` saat ini) adalah kebijakan yang benar dan sudah final** — bukan bug, bukan regresi yang perlu diperbaiki. Dokumen `docs/superpowers/specs/...` untuk sub-project 03 dianggap usang/superseded pada poin ini.
