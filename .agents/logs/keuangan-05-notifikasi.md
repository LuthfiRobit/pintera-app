...
1. **Bug retroaktif di Task 4 (ditemukan saat Task 5):** `NotificationDispatcher::logAttempt()` menulis `notification_logs.user_id = $notifiable->id` tanpa memandang tipe notifiable — salah untuk `OrangTua` (yang `id`-nya BUKAN `users.id`, ada kolom `user_id` terpisah). Ini BUKAN sekadar crash — pada kasus ID kebetulan bertabrakan, log salah dicatat ke user yang SALAH SAMA SEKALI (masalah integritas data/privasi-adjacent), baru crash (FK violation) kalau ID tidak kebetulan cocok. Task 4 lolos test karena kebetulan ID cocok. Dibuktikan secara empiris oleh reviewer (revert-lalu-restore fix, lihat bukti tetap menulis user salah). Diperbaiki di Task 5, dikunci dengan regression test baru di ronde perbaikan whole-plan (lihat di bawah).
...
Semua 14 commit ada di `demo` langsung: `28bc293..ecd289e`. **Belum di-push ke remote**, sesuai instruksi eksplisit user di awal eksekusi sub-project ini.
