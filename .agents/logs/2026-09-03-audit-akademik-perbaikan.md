...
**Keputusan user (2026-09-03): Plan Akademik (Kenaikan Kelas) ditutup SELESAI.** Poin tingkat-validasi dan gap `kelas_id` siswa Keluar/Pindah (`kelas_terakhir_id` + 5 titik konsumen) resmi jadi spec terpisah untuk digarap nanti, TIDAK digabung ke plan ini.

### Pengecualian: tambalan minimal terisolasi untuk 1 dari 6 titik konsumen (bug produksi aktif)

`SubmitPengajuanRaporAction.php:37` bukan cuma risiko masa depan — ini **memblokir submit rapor wali kelas HARI INI** kalau ada siswa Keluar/Pindah yang `kelas_id`-nya belum dibersihkan (guaranteed terjadi, karena `updateStatus()` memang tidak pernah membersihkannya). User memutuskan ini tidak boleh menunggu spec besar `kelas_terakhir_id` yang lintas-modul.

**Ditambal** (commit lihat riwayat git terbaru): tambah `->where('status', StatusSiswa::Aktif->value)` ke query `$kelas->siswa()` di `SubmitPengajuanRaporAction::execute()` — pola identik dengan `PresensiAggregationService.php:20` dan pola `->value` (bukan bare enum) yang konsisten dipakai di seluruh codebase (`SiswaController`, `VirtualAccountController`, dll). Tidak menyentuh skema/migration apapun — murni filter query.

**Test regresi**: `tests/Feature/Akademik/SubmitPengajuanRaporActionTest.php` — test baru `'does not let a siswa Pindah/Keluar whose kelas_id was never cleared block submission, and still processes active siswa normally'`, dengan 1 kelas berisi 2 siswa aktif (sudah lengkap catatan) + 1 siswa `Keluar` (`kelas_id` masih menempel, sengaja tanpa `CatatanWaliKelas`). **Diverifikasi ketat**: sebelum fix diterapkan (`git stash` sementara ke kode lama), test ini GAGAL dengan `ValidationException` persis seperti bug yang dilaporkan — bukti test benar-benar menangkap bug, bukan sekadar lolos kebetulan. Regresi modul Rapor penuh: 137/137 passed.

**PENTING — batas scope, JANGAN diperluas tanpa spec resmi**: 5 titik konsumen lain yang sama-sama tidak filter status (`RaporCalculationService.php:27`, `Guru\RaporController.php:94` dan `:143`, `DashboardStatsService.php:138`, `JenisTagihanSasaranMatcher.php` + `TagihanBillingGenerator` di modul Keuangan) **SENGAJA TIDAK ikut ditambal di sini**. Semuanya menunggu spec `kelas_terakhir_id` yang lebih permanen (kolom snapshot + pembersihan `kelas_id` di sumber `updateStatus()`). Kalau ada yang tergoda menambal salah satu dari 5 titik ini secara terpisah sebelum spec besar itu jalan, STOP dulu — akan menghasilkan 2 pola perbaikan (tambal cepat vs solusi permanen) yang saling tumpang tindih dan membingungkan saat spec besar akhirnya digarap.
