...
2. **Hero**: eyebrow badge (musim penerimaan aktif), headline, deskripsi, 2 CTA ("Lihat Lembaga & Jalur" primer → scroll ke `#lembaga`, "Cek Status Pendaftaran" outline → **juga** scroll ke `#lembaga`, bukan langsung ke `CekStatusController` — controller itu wajib punya `{lembagaSlug}` di URL-nya dan redesign-nya sengaja ditunda, jadi welcome page yang levelnya global tidak bisa menebak lembaga mana yang dimaksud; link nyata ke `CekStatusController` per-lembaga ditaruh di halaman Detail Lembaga, lihat §3.2). Panel navy di kanan: ringkasan (jumlah lembaga, jumlah sedang buka, jumlah jalur aktif — dihitung real dari DB, bukan hardcode), catatan gelombang yang sedang berjalan (ambil dari `GelombangPpdb` yang `tanggal_buka <= now() <= tanggal_tutup`, lembaga & tanggal tutup terdekat), tombol "Mulai Pendaftaran" → scroll ke `#lembaga`.
...
4b. **Link "Cek status pendaftaran di lembaga ini"**: link kecil (bukan tombol utama) di dekat header profil lembaga, mengarah ke `route('spmb.status.form', ['lembagaSlug' => $lembaga->slug])` — `CekStatusController` yang sudah ada, tidak diubah. Ini satu-satunya tempat di sub-project ini yang benar-benar menyambung ke fitur cek-status, karena baru di level lembaga URL-nya bisa resolve.
5. **Section jalur**: grid 3 kolom (`JalurPpdb::where('status_aktif', true)` untuk `TahunAjaran` aktif lembaga itu — bukan `JalurPpdb` dari tahun ajaran lama/tidak aktif), diurutkan berdasarkan `id` ascending (urutan seeding/pembuatan). **Aturan pembatasan jalur-per-gelombang yang sudah ada di `PortalController::index` (kode lama, wajib dipertahankan persis):** kalau lembaga sedang punya gelombang aktif (`PortalController::cariGelombangAktif($lembaga)`) dan gelombang itu punya baris di pivot `gelombang_jalur` (`$gelombang->jalur()->exists()`), maka jalur yang ditampilkan **cuma yang terhubung ke gelombang itu** (`JalurPpdb::whereHas('gelombang', fn ($q) => $q->whereKey($gelombang->id))`) — bukan semua jalur aktif lembaga. Kalau gelombang tidak punya baris pivot sama sekali (tidak dibatasi), atau memang tidak ada gelombang aktif, tampilkan semua `JalurPpdb` aktif tahun ajaran itu tanpa filter tambahan.

   Jalur bernama persis "Reguler" (kalau ada di lembaga itu) ditandai `.featured` dengan ribbon "Paling Umum" — aturan ini berbasis nama, bukan posisi/urutan, supaya tidak arbitrer; kalau lembaga tidak punya jalur bernama "Reguler", tidak ada card yang di-featured sama sekali. Tiap card jalur:
