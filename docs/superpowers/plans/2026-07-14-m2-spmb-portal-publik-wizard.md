...
- Uploaded files during the wizard (before final submit) are stored under `storage/app/public/pendaftaran-tmp/{session id}/` using Laravel's default randomized filename (`UploadedFile::store()`), and moved to `storage/app/public/pendaftaran/{pendaftaran id}/{basename of the stored temp file}` only at final submit. The wizard session tracks the actual `file_path` for each `dokumen_syarat_ppdb_id` directly (see Task 3's `dokumen` session shape) — later steps must always read the path from session data, never reconstruct it from a naming convention.
