...
**Tech Stack:** Laravel 12.x, PHP 8.3+, MySQL 8+, Laravel Breeze (Blade stack), Tailwind CSS, Alpine.js, Pest 4.x (see Task 2 amendment below), spatie/laravel-permission, spatie/laravel-activitylog.
...
- Testing: Pest 4.x (amended during Task 2 execution — Laravel Breeze's installer forces a transitive upgrade from the originally-pinned Pest ^3.0 to ^4.7; all Task 1/2 tests pass unchanged under 4.x, and nothing in this plan's test code uses a Pest-3-only API, so the upgrade was accepted rather than pinning an older Breeze release), `RefreshDatabase` di semua Feature test
