...
- **[UI/UX]** Merombak halaman Create & Edit Pengguna (`users`) menjadi *Gold Standard SPA Profile*, dengan mengekstrak input ke `_form.blade.php` dan merancang navigasi *Tab* "Profil & Identitas" via Alpine.js (`tabs/profil.blade.php`).
- **[UI/UX]** Mengaplikasikan desain *Hero Profile Card* dengan inisial nama, *badge* status, dan efek *glassmorphism* di halaman Edit Pengguna.
- **[UI/UX]** Merombak arsitektur halaman Edit Role menjadi tata letak **Sticky Sidebar Premium** (2 kolom). Form nama dan *scope* tetap menempel di kiri layar (*sticky top-6*) sehingga mudah dijangkau saat menggulir *Permission Matrix* di kanan.
- **[UI/UX]** Mempercantik *Permission Matrix* dengan memisahkannya ke dalam modul-modul bersayap tipis (`rounded-xl border-gray-200 bg-gray-50/50`) dan mengubah skema usang (`ink`, `brass`) ke komponen *Tailwind* murni (`gray-900`, `brand-600`, dan `amber-50`).
- **[BUGFIX]** Memperbaiki eror penguraian (parsing) pada `x-bind:class="syncing ? 'animate-spin' : ''"` di dalam komponen `<x-icon>` Laravel Blade pada halaman Edit Role yang sebelumnya menganggap Alpine JS state sebagai variabel PHP (*Undefined constant "syncing"*).
- **[UI/UX]** Menyelaraskan *Header* navigasi atas (*Breadcrumbs* + tombol Kembali) pada `roles/edit.blade.php` dan merombak halaman `roles/create.blade.php` agar sama-sama menggunakan gaya visual elegan dari *Sticky Sidebar*.
